...
En la imagen se han dispuesto la asignación de las $P$ y $Q$ en forma vertical conectadas con una trazo simple. Este tipo de asignaciones son conocidas como L-sistemas que son gramática a nivel computacional empleados principalmente para modelar el proceso de crecimiento de las plantas
:::
...
$$
La expresión $a_n = j + nd$ corresponde con la solución de la relación de recurrencia ya que permite encontrar cualquier término del conjunto $\{ a_n\}$ asignado un valor de $n$.  

El comportamiento se este conjunto de datos es lineal y para nuestra comprensión lo entenderemos como el caso en que la diferencia entre el valor presente y pasado es siempre constante.
::: 
:::
...
$$
De manera similar al ejemplo anterior, la expresión $a_n = r^n j$ es la solución de la relación de recurrencia ya que permite encontrar cualquier término del conjunto $\{ a_n\}$ asignado un valor de $n$. 

El comportamiento de este conjunto de dice exponencial y para nuestra comprensión lo entenderemos como el caso en que el valor presente es proporcional al valor pasado, o bien, cuando el cociente del valor presente y pasado permanece constante. 
:::
:::

Con el siguiente código graficamos un conjunto de $20$ puntos cuyo comportamiento es exponencial, aquí se ha tomado $r=1.05$ y $a_0 = 2$
```{python}
...
for n in range(1 , len(x) ):
  a.append( r*a[n-1] )   # relación de recurrencia en la forma r*a_(n-1)
...
:::
:::
Con el siguiente código graficamos un conjunto de $10$ puntos cuyo comportamiento se acerca de forma oscilante a $p=5$, aquí se ha tomado $a_0 = 24.$
...
$h.$  $\dfrac{7}{2}, \dfrac{5}{12} , \dfrac{17}{8}, \dfrac{1}{2}, \dfrac{27}{14}    \ldots$
:::
...
Una sucesión $\{ a_n \}$ es creciente si $a_n \leq  a_{n+1}$ para todo $n$ y es decreciente si $a_n \geq a_{n+1}$ para todo $n$. 

En general diremos que una sucesión es monótona si esta es crecientes o decreciente. 
:::
:::
...
:::
::: {.proof}
Supongamos sin pérdida de generalidad que la sucesión es creciente. Como el conjunto $\{ a_n \}$ es acotado, la menor de las cotas superiores de este conjunto, digamos $p$, verifica que $a_n < p$ para todo $n$ y que dado un $\epsilon > 0$ de nuestra elección existe al menos un elemento $a_N$ tal que 
$$
p - \epsilon < a_N,
$$
como la sucesión es creciente entonces la desigualdad anterior es válidad para todo $n > N$ con lo que
$$ 
p - \epsilon < a_n < p 
$$
o equivalentemente 
...
$$
para todo $n > N,$ y de acuerdo con esto $\{a_n \}$ es convergente a $p$.
:::
...
##
:::{#def-asinto_equiv}
Dos sucesiones $\{ a_n \}$ y $\{ b_n \}$ se dicen asintoticamente equivalentes si
$$
...
$$
a_n = \left( n^s + n + u \right)^t \text{ y } b_n = \left( n^p + n + v \right)^q
$$
con $s > 1$,  $p > 1$ y $st = pq$.
:::
:::
